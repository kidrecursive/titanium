				...
				async.whilst(
					...
					function (callback) {
						...
						appc.async.series(this, orderedOptionNames.map(function (name) {
							return function (cb) {
								if (prompting && (missingCount || invalidCount)) return cb();
								if (options[name].validated) return cb();
...
CLI.prototype.prompt = function prompt(items, done) {
	// sanity check
	if (!items || (Array.isArray(items) && (!items.length || !items.some(function (i) { return i; })))) {
		return done();
	}
